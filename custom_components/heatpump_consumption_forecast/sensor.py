...
class HeatPumpDataQualitySensor(HeatPumpStringSensor):
    def __init__(self, coordinator: HeatPumpForecastCoordinator, entry: ConfigEntry) -> None:
        super().__init__(coordinator, entry, "confidence", "Datenqualität", "mdi:database-check-outline")

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        structured = self.coordinator.data.get("reason_structured") or {}
        return {"Abgeschlossene_Tagesdaten": self.coordinator.data.get("confidence_completed_days"), "Temperaturhistorie_Tage": (structured.get("temperature_history") or {}).get("days"), "Verbrauchshistorie_Tage": (structured.get("history") or {}).get("days"), "Heizkurve": self.coordinator.data.get("heating_curve"), "ML_Status": self.coordinator.data.get("ml_status")}
